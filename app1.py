...
DATE_PATTERN = r'(?:Invoice\sDate|Order\sDate|Date|DATE|date)[:\s]*([0-9]{1,2}[/-][0-9]{1,2}[/-][0-9]{2,4})|(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})'
ORDER_PATTERN = r'(?:Order|ORD|order|ORDERS|orders|Order\sNo|Order\sNumber)[:\s]*([A-Z0-9-]+)'
INVOICE_PATTERN = r'(?:Invoice\sNumber|Invoice|INVOICE|invoice|Inv|inv|INVOICE\sNo)[:\s]*([A-Z0-9-]+)'
COMPANY_PATTERN = r'(?:From|Supplier|Company|trading name of)[:\s]*([A-Za-z0-9 &.,]+)'
